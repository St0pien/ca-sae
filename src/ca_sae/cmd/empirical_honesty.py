...
#         # Uniform k must be integer for this hard post-hoc annotation.
#         k_int = int(round(rho))
...
#     train_A:
#         Empirical feature-class selection frequencies on training data.

#     test_A:
#         Empirical feature-class selection frequencies on held-out data.

#     posthoc_M:
#         Binary post-hoc feature-class annotation constructed from train_A.

#     k:
#         Per-feature number of claimed classes.

#     Reports:
#         - cosine(posthoc_M[i], test_A[i])
#         - precision / recall / F1 of claimed classes against top-k
#           empirical test classes
#         - per-feature metrics
#     """
...
#         # Estimated mode can produce features with zero associations.
#         if ki == 0:
#             continue
...
#         "recall": recall,
#         "f1": f1,
#         "valid_set_metrics": valid_set_metrics,
#     }
...
#     f1 = metrics["f1"]
#     valid_set_metrics = metrics["valid_set_metrics"]

#     valid_precision = precision[valid_set_metrics]
#     valid_recall = recall[valid_set_metrics]
#     valid_f1 = f1[valid_set_metrics]
...
#         "fraction_zero_budget_features": num_zero_budget / d,
#         "mean_cosine": nanmean(cosine[valid_cosine]),
#         "median_cosine": float(cosine[valid_cosine].median()),
#         "mean_precision": nanmean(valid_precision),
#         "mean_recall": nanmean(valid_recall),
#         "mean_f1": nanmean(valid_f1),
#     }
...
#         f"{num_zero_budget:,} ({100 * num_zero_budget / d:.2f}%)"
#     )
#     print()
#     print(f"Mean cosine(M, A_test): " f"{cosine[valid_cosine].mean().item():.4f}")
#     print(f"Median cosine(M, A_test): " f"{cosine[valid_cosine].median().item():.4f}")
#     print()
#     print(f"Precision @ k_i:        " f"{nanmean(valid_precision):.4f}")
#     print(f"Recall @ k_i:           " f"{nanmean(valid_recall):.4f}")
#     print(f"F1 @ k_i:               " f"{nanmean(valid_f1):.4f}")
...
def evaluate_train_test_agreement(
    ...
    cosine = (posthoc_M * test_A).sum(dim=1) / (
        M_norm * A_norm
    ).clamp_min(1e-12)
...
def main(
    ...
    print(
        f"  Active only:            "
        f"{results['mean_cosine_active']:.4f}"
    )
    print(
        f"  All features:           "
        f"{results['mean_cosine_all']:.4f}"
    )
    print(
        f"  Median active:          "
        f"{results['median_cosine_active']:.4f}"
    )
    print(
        f"  Median all:             "
        f"{results['median_cosine_all']:.4f}"
    )
    ...
    print(
        f"  Active only:            "
        f"{results['mean_precision_active']:.4f}"
    )
    print(
        f"  All features:           "
        f"{results['mean_precision_all']:.4f}"
    )
    ...
    print(
        f"  Active only:            "
        f"{results['mean_recall_active']:.4f}"
    )
    print(
        f"  All features:           "
        f"{results['mean_recall_all']:.4f}"
    )
    ...
    print(
        f"  Active only:            "
        f"{results['mean_f1_active']:.4f}"
    )
    print(
        f"  All features:           "
        f"{results['mean_f1_all']:.4f}"
    )
        ...
        per_feature.append(
            {
                ...
                "cosine": (
                    None
                    if not torch.isfinite(cosine_cpu[i])
                    else float(cosine_cpu[i])
                ),
                ...
                "precision": (
                    None
                    if torch.isnan(precision_cpu[i])
                    else float(precision_cpu[i])
                ),
                "recall": (
                    None
                    if torch.isnan(recall_cpu[i])
                    else float(recall_cpu[i])
                ),
                "f1": (
                    None
                    if torch.isnan(f1_cpu[i])
                    else float(f1_cpu[i])
                ),
...
def cli():
    ...
    parser.add_argument(
        ...
        default=5.0,
        help=(
            "Average number of class associations per feature. "
            "Defaults to 5."
        ),
    )
